{ self, inputs, ... }: {
  flake = {
    nixosModules.nvim = { config, pkgs, lib, ... }: {
    ...
    homeManagerModules.nvim = { config, pkgs, lib, systemTheme, nvimProfile, ... }: let
      colorschemes = {
        nord = {
        ...
        minimalist = {
          ...
          module = "zen";
          priority = 1000;
          opts = { variant = "dark"; transparent = true; };
          colorscheme = "zen";
        };
      };
      ...
      toLua = value:
        if builtins.isString value then "\"${value}\""
          ...
          + " }"
        else abort "unsupported value for nvim colorscheme opts";
      renderPlugin = c: extra: let
        parts = [ ''"${c.repo}"'' ]
      ...
      colorschemeLua =
        if nvimProfile == "full" then
          ''
            return {
              ${renderPlugin selected []},
              { "LazyVim/LazyVim", opts = { colorscheme = "${selected.colorscheme}" } },
            }
          ''
        else
          let
            setupLua = "local ok, mod = pcall(require, \"${selected.module}\"); "
      ...
      activePath = "/home/grae/nixos/modules/features/nvim"
      ...
      configFiles = builtins.listToAttrs (map (f: {
        name = "nvim/${f}";
